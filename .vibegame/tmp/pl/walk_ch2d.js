...
const posOf = (id) => { const p = (S().positions || []).find((x) => x.instrumentId === id); return p ? p.qty + '/' + p.lockedQty : '0/0'; };
const sv = () => ({
  beatId: C().beatId, idx: C().beatIndex, date: S().inGameDate, open: S().isMarketOpen,
  reqs: C().beatRequirements.map((r) => r.id + '=' + r.satisfied),
  last: S().lastOrder && (S().lastOrder.reasonCode),
  pos: posOf('601398'), ruleCards: C().ruleCardsSeen.slice(), ans: C().pendingChoice && C().pendingChoice.answeredKey,
...
log.push({ t: 'ch1 confirm', r: b.chapterConfirm() });
snap('entered ch2');
...
snap('2.7');
b.chapterRead('panel.eventCard', 1); b.chapterClosePanel('panel.eventCard');
snap('after2.7');
log.push({ t: 'chapterEnd', endPhase: S().endPhase, rating: C().rating, extra: C().extraSegment && C().extraSegment.kind, completed: C().completedBeats.length, remaining: C().remainingBeats });
return log;
